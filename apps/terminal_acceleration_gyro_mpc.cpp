...
int main(int argc, char** argv) {
    ...
    try {
        ...
        gtsam::Vector6 target_pose_sigmas;
        target_pose_sigmas <<
        ...
        const auto target_pose_noise = gtsam::noiseModel::Diagonal::Sigmas(target_pose_sigmas);
        const auto target_velocity_noise = gtsam::noiseModel::Isotropic::Sigma(
            3, config["terminal_velocity_sigma"].as<double>());
        gtsam::Vector9 dynamics_sigmas;
        dynamics_sigmas <<
            gtsam::Vector3::Constant(config["dynamics_position_sigma"].as<double>()),
            gtsam::Vector3::Constant(config["dynamics_rotation_sigma"].as<double>()),
            gtsam::Vector3::Constant(config["dynamics_velocity_sigma"].as<double>());
        const auto dynamics_noise = gtsam::noiseModel::Diagonal::Sigmas(dynamics_sigmas);
        const auto control_noise = gtsam::noiseModel::Diagonal::Sigmas(controlSigmas(
        ...
        for (std::size_t iteration = 0; iteration < mpc_iterations; ++iteration) {
            ...
            const UAVFactor::DynamicsState terminal_reference = circleReference(
                circle_center, circle_radius, circle_angular_speed, time + horizon * dt,
                face_velocity);
            ...
            graph.add(gtsam::PriorFactor<gtsam::Pose3>(X(horizon), terminal_reference.pose,
                                                        target_pose_noise));
            graph.add(gtsam::PriorFactor<gtsam::Vector3>(V(horizon), terminal_reference.velocity,
                                                          target_velocity_noise));
            graph.add(UAVFactor::TerminalAccelerationGyroFactor(
                X(0), V(0), control_keys, X(horizon), V(horizon), dt, dynamics_noise));
            ...
            initial_values.insert(X(horizon), terminal_reference.pose);
            initial_values.insert(V(horizon), terminal_reference.velocity);
            for (std::size_t k = 0; k < horizon; ++k)
        ...
        std::cerr << "terminal_acceleration_gyro_mpc: " << exception.what() << '\n';
        return 1;
    }
}
