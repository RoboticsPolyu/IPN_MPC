...
int main(int argc, char** argv) {
    ...
    double scmpc_kf = quad_config["scmpc_kf"].as<double>();
    ...
    std::string file_name = "../data/log/SCMPC_";
    ...
    Trajectory::vertical_circle_generator fig_gen(radius, linear_vel,
                                                  dt); // scale=1m, speed=1rad/s, dt=0.01s
    ...
    state.t = 0.0;
    state.pos = gtsam::Vector3(0.0, 0.0, 0.0);
    state.vel = gtsam::Vector3(0.0, 0.0, 0.0);
    state.rotation = gtsam::Rot3::Identity().toQuaternion();
    ...
    for (int traj_idx = 0; traj_idx < sim_steps; traj_idx++) {
        ...
        if (traj_idx == 0) {
            ...
            predicted_state.p = fig_gen.pos(0.0) + gtsam::Vector3(0.0, -0.05f, 0.0f);
        ...
        for (int idx = 0; idx < opt_lens_traj; idx++) {
            DynamicsFactorTGyroWall dynamics_factor(X(idx), V(idx), U(idx), X(idx + 1), V(idx + 1),
                                                    dt, mass, drag_k, dynamics_noise);
        ...
        double external_force = (est_state.p.y() + 0.05f) * scmpc_kf;
        gtsam::Vector3 v_dot =
            -gtsam::Vector3(0, 0, g_) +
            est_state.rot.rotate(gtsam::Vector3(0, 0, (input[0] + at_noise) / mass)) +
            drag_force / mass + gtsam::Vector3(0, external_force / mass, 0);

        IPN_LOG_DEBUG << "External force [N]: " << external_force;
