...
int main(int argc, char** argv) {
    ...
    auto dynamics_noise = noiseModel::Diagonal::Sigmas(
        (Vector(12) << Vector3::Constant(0.005), Vector3::Constant(0.005), Vector3::Constant(0.005),
         Vector3::Constant(0.005))
            .finished());

    // Initial state noise
    auto vicon_noise = noiseModel::Diagonal::Sigmas(
        (Vector(6) << Vector3::Constant(0.001), Vector3::Constant(0.001)).finished());
    ...
    auto ref_predict_pose_noise = noiseModel::Diagonal::Sigmas(
        (Vector(6) << Vector3::Constant(0.1), Vector3::Constant(0.03)).finished());
    ...
    Quadrotor quad_(simulator_config_path);
    ...
    for (int traj_idx = 0; traj_idx < 100000; traj_idx++) {
        ...
        if (traj_idx == 0) {
            for (int idx = 0; idx < opt_lens_traj; idx++) {
                if (idx == 0) {
                    ...
                    newFactors.add(gtsam::PriorFactor<gtsam::Pose3>(
                        X(idx), gtsam::Pose3(predicted_state.rot, predicted_state.p), vicon_noise));
                    newFactors.add(
                        gtsam::PriorFactor<gtsam::Vector3>(V(idx), predicted_state.v, vel_noise));
                    newFactors.add(gtsam::PriorFactor<gtsam::Vector3>(
                        S(idx), predicted_state.body_rate, omega_noise));
                ...
                gtsam::Pose3 pose_idx(
                    gtsam::Rot3::Expmap(circle_generator.theta(t0 + (idx + 1) * dt)),
                ...
                // newFactors.add(gtsam::PriorFactor<gtsam::Vector4>(U(idx), init_input,
                // input_noise));
                ...
                newFactors.add(gtsam::PriorFactor<gtsam::Vector3>(S(idx + 1), omega_idx,
                                                                  ref_predict_omega_noise));

                // FactorIndices contains there initial state prior factor (3), dynamics factor (1)
                // + control state prior (3) FactorIndices sum : 4 * 20 + 3 = 83
            }
            ...
            DynamicsFactorTm dynamics_factor(X(idx), V(idx), S(idx), U(idx), X(idx + 1), V(idx + 1),
                                             S(idx + 1), dt, dynamics_noise);
            newFactors.add(dynamics_factor);
            ...
            newFactors.add(
                gtsam::PriorFactor<gtsam::Vector3>(V(idx + 1), vel_idx, ref_predict_vel_noise));
            newFactors.add(
                gtsam::PriorFactor<gtsam::Vector3>(S(idx + 1), omega_idx, ref_predict_omega_noise));

            /* Add current state measurement */
            IPN_LOG_DEBUG << "State measurement added: estimate_index=" << cur_est_idx;
            newFactors.addPrior(X(cur_est_idx),
                                gtsam::Pose3(predicted_state.rot, predicted_state.p), vicon_noise);
            ...
            // FactorIndices: adding newest control state prior (3), then current estimation state
            // prior (3) FactorIndices = 83 + (3 + 3) * cur_est_idx
        }
        ...
        for (uint32_t ikey = cur_est_idx; ikey < opt_lens_traj + cur_est_idx; ikey++) {
            ...
            gtsam::Vector3 ref_vel = circle_generator.vel(
                t0 + ikey * dt); //(gtsam::Rot3::Expmap(circle_generator.theta(ikey*
                                 // dt)), circle_generator.pos(ikey * dt));
            IPN_LOG_DEBUG << "Reference velocity [m/s]: \n" << ref_vel.transpose();

            omega = result.at<Vector3>(S(ikey));
            IPN_LOG_DEBUG << "Optimized angular velocity [rad/s]: \n" << omega.transpose();
            gtsam::Vector3 ref_omega = circle_generator.omega(
                t0 + ikey * dt); //(gtsam::Rot3::Expmap(circle_generator.theta(ikey*
                                 // dt)), circle_generator.pos(ikey * dt));
        ...
        gtsam::Vector3 err;
        err.Zero();
        if (!quad_.renderHistoryOpt(opt_trj, err)) {
            break;
        }
