...
class back_and_forth_generator {
  public:
    back_and_forth_generator(double length, double max_speed, double dt)
        : length_(length), max_speed_(max_speed), dt_(dt) {
        // 计算达到最大速度所需的时间和距离
        ...
        // 调试信息
        IPN_LOG_DEBUG << "Back-and-forth trajectory initialized: length_m=" << length_
                      << ", max_speed_mps=" << max_speed_ << ", max_acceleration_mps2=" << max_acc_
                      << ", acceleration_time_s=" << acc_time_
                      << ", acceleration_distance_m=" << acc_distance_
                      << ", cruise_time_s=" << cruise_time_ << ", one_way_time_s=" << one_way_time_;
    };
  ...
  private:
    ...
    // 辅助函数：计算在单程内的运动状态
    void get_segment_state(double t, int& direction, double& normalized_time, double& s, double& v,
                           double& a);
};
...
class figure_eight_generator {
  public:
    figure_eight_generator(double scale, double speed, double dt)
        : scale_(scale), speed_(speed), dt_(dt){};
