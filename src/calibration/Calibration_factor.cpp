...
namespace UAVFactor
{
    ...
    DynamcisCaliFactor_RS::DynamcisCaliFactor_RS(Key p_i, Key vel_i, Key omega_i,
        Key p_j, Key vel_j, Key omega_j, Key im_key, Key rwg_key, Key p_key, Key kf_key, Key km_key, Key bTm_key, gtsam::Vector4 actuator_outputs, float dt, float mass, const SharedNoiseModel &model)
        : Base(model, p_i, vel_i, omega_i, p_j, vel_j, omega_j, im_key, rwg_key, p_key, kf_key, km_key, bTm_key)
        , dt_(dt)
        , mass_(mass)
    ...
    Vector DynamcisCaliFactor_RS::evaluateError(const gtsam::Pose3 &pos_i, const gtsam::Vector3 &vel_i, const gtsam::Vector3 &omega_i, 
                             ...
                             boost::optional<Matrix &> H11, boost::optional<Matrix &> H12) const
    {
        gtsam::Vector12 err;
