...
namespace UAVFactor
{
        ...
        boost::optional<Matrix &> H9) const
    {
        ...
        if (H8)
        {
            Matrix123 J_e_inertia_moment;
            J_e_inertia_moment.setZero();
            ...
            J_e_inertia_moment.block(9, 0, 3, 3) = Jac_Jinv* dt_ + Jac_jb* dt_;

            *H8 = J_e_inertia_moment;
        }
        ...
        boost::optional<Matrix &> H9) const
    {
        ...
        // position rotation velocity error
        gtsam::Matrix33 J_pe_rwg, J_pe_roti;
        gtsam::Vector3  p_err = mass_* r_w_mi.unrotate(p_w_mj - vel_i * dt_ + 0.5f* gI_* dt2 - p_w_mi, J_pe_roti) - 0.5f* thrust_torque.head(3)* dt2;
        gtsam::Matrix33 J_rerr_rbj, J_rbi, J_gI;
        gtsam::Vector3  rot_err = Rot3::Logmap(r_w_mj.between(r_w_mi.compose(Rot3::Expmap(omega_i * dt_), J_rbi), J_rerr_rbj));
        ...
        if (H1)
        {
            Matrix33 Jac_perr_p = - mass_* r_w_mi.inverse().matrix();
        ...
        if (H2)
        {
            Matrix123 J_e_v;
            J_e_v.setZero();
            Matrix33 Jac_perr_veli  = - mass_* r_w_mi.inverse().matrix() * dt_;
            Matrix33 Jac_verr_v     = - mass_* r_w_mi.inverse().matrix();
            J_e_v.block(0, 0, 3, 3) = Jac_perr_veli;
            J_e_v.block(6, 0, 3, 3) = Jac_verr_v;
        ...
        if (H4)
        {
            ...
            J_e_thrust_moments.block(0, 0, 3, 3) = - I_3x3* dt2* 0.5f;
        ...
        if (H5)
        {
            Matrix126 J_e_posej;
            J_e_posej.setZero();
            J_e_posej.block(0, 0, 3, 6) = mass_* r_w_mi.inverse().matrix()* jac_t_posej;
            J_e_posej.block(3, 0, 3, 6) = J_rerr_rbj * jac_r_posej;
            *H5 = J_e_posej; 
        }
        ...
        if (H6)
        {
            Matrix123 J_e_vj;
            J_e_vj.setZero();
            J_e_vj.block(6, 0, 3, 3) = mass_* r_w_mi.inverse().matrix();
            *H6 = J_e_vj;
        }
        ...
        if (H8)
        {
            ...
            J_e_inertia_moment.block(9, 0, 3, 3) = _omega + Jac_jb* dt_;
        ...
        if (H9)
        {
            Matrix123 J_e_rwg;
            J_e_rwg.setZero();
            J_e_rwg.block(0, 0, 3, 3) = mass_* r_w_mi.inverse().matrix() * 0.5f * dt2;
            J_e_rwg.block(6, 0, 3, 3) = mass_* r_w_mi.inverse().matrix() * dt_;
            *H9 = J_e_rwg;
        }
