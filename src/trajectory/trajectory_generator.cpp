...
gtsam::Vector3 circle_generator::vel(double t) {
    ...
    // double v_x = (sin(angular_speed * (t + dt_)) * radius_ - sin(angular_speed * (t)) * radius_)
    // / (dt_); double v_y = (cos(angular_speed * (t + dt_)) * radius_ - cos(angular_speed * (t)) *
    // radius_) / (dt_);

    gtsam::Vector3 vel = gtsam::Vector3(cos(angular_speed * t) * angular_speed * radius_,
                                        -sin(angular_speed * t) * angular_speed * radius_, 0);
    return vel;
}
...
gtsam::Vector3 circle_generator::omega(double t) {
    // return gtsam::Rot3::Logmap(gtsam::Rot3::Expmap(theta(t -
    // dt_)).between(gtsam::Rot3::Expmap(theta(t + dt_)) ) ) / (2*dt_);
...
gtsam::Vector3 circle_generator::thrust(double t) {
    ...
    force = gtsam::Vector3(-sin(angular_speed * t) * angular_speed * angular_speed * radius_,
                           -cos(angular_speed * t) * angular_speed * angular_speed * radius_, 0) +
            g_;
...
gtsam::Vector4 circle_generator::inputfm(double t) {
    ...
    // IPN_LOG_DEBUG << "Tmb: " << Tmb.transpose();
    return Tmb;
}
...
gtsam::Vector4 circle_generator::input(double t) {
    ...
    K1 << dynamics_params_.k_f, dynamics_params_.k_f, dynamics_params_.k_f, dynamics_params_.k_f,
        dx0 * dynamics_params_.k_f, -dx0 * dynamics_params_.k_f, -dx0 * dynamics_params_.k_f,
        dx0 * dynamics_params_.k_f, -dx1 * dynamics_params_.k_f, -dx1 * dynamics_params_.k_f,
        dx0 * dynamics_params_.k_f, dx0 * dynamics_params_.k_f, dynamics_params_.k_m,
...
gtsam::Vector3 vertical_circle_generator::vel(double t) {
    ...
    gtsam::Vector3 vel = gtsam::Vector3(cos(angular_speed * t) * angular_speed * radius_, 0.0,
                                        -sin(angular_speed * t) * angular_speed * radius_);
    return vel;
}
...
gtsam::Vector3 vertical_circle_generator::thrust(double t) {
    gtsam::Vector force;
    double angular_speed = speed_ / radius_;
    force = gtsam::Vector3(-sin(angular_speed * t) * angular_speed * angular_speed * radius_, 0.0,
...
gtsam::Vector3 cir_conacc_generator::vel(double t) {
    ...
    gtsam::Vector3 vel = gtsam::Vector3(cos(angle) * angular_acc * t * radius_,
                                        -sin(angle) * angular_acc * t * radius_, 0);
    // IPN_LOG_DEBUG << "Analytical vel: " << vel.transpose();

    // vel = (pos(t + dt_) - pos(t)) / dt_; // better than Analytical solution ???
    // IPN_LOG_DEBUG << "Diff vel:     " << vel.transpose();
...
gtsam::Vector3 cir_conacc_generator::omega(double t) {
    return gtsam::Rot3::Logmap(
               gtsam::Rot3::Expmap(theta(t)).between(gtsam::Rot3::Expmap(theta(t + dt_)))) /
           dt_;
}
...
gtsam::Vector4 cir_conacc_generator::input(double t) {
    ...
    // K1 << dynamics_params_.k_f, dynamics_params_.k_f, dynamics_params_.k_f, dynamics_params_.k_f,
    //     0, 0, dynamics_params_.arm_length * dynamics_params_.k_f, -dynamics_params_.arm_length *
    //     dynamics_params_.k_f, -dynamics_params_.arm_length * dynamics_params_.k_f,
...
gtsam::Vector3 figure_eight_generator::pos(double t) {
    ...
    return gtsam::Vector3(scale_ * sin(tau), scale_ * sin(tau) * cos(tau),
...
gtsam::Vector3 figure_eight_generator::vel(double t) {
    ...
    return gtsam::Vector3(scale_ * speed_ * cos_tau, scale_ * speed_ * cos(2 * tau), 0.0);
}
...
void back_and_forth_generator::get_segment_state(double t, int& direction, double& normalized_time,
    ...
    if (normalized_time < acc_time_) {
        ...
        s = length_ -
            0.5 * max_acc_ * (decel_time - decel_start_time) * (decel_time - decel_start_time);
