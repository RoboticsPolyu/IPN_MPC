...
namespace UAVFactor
{
        ...
        ActuatorEnabled &actuatorEnabled, boost::optional<gtsam::Matrix &> Jacobian)
    {
        ...
        for (int rotor_idx = 0; rotor_idx < geometry.num_rotors; rotor_idx++)
        {
            ...
            float km = geometry.rotors[rotor_idx].torque_ratio;
            ...
            // Compute torque generated by this rotor
            gtsam::Vector3 torque = ct * position.cross(axis) - ct * km * axis;

            // Fill corresponding items in effectiveness gtsam
            for (size_t j = 0; j < 3; j++)
            {
                effectiveness(j, rotor_idx) = torque(j);
    ...
    Vector AllocationFactor::evaluateError(const double& thrust, const gtsam::Vector3 &torques, const gtsam::Vector4 &actuator_outputs,
