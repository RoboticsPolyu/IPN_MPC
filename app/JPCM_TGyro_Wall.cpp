...
int main(void)
{
    ...
    Trajectory::vertical_circle_generator fig_gen(radius, linear_vel, dt);  // scale=1m, speed=1rad/s, dt=0.01s
    ...
    for(int traj_idx = 0; traj_idx < SIM_STEPS; traj_idx++)
    {
        ...
        {
            DynamicsFactorTGyroWall dynamics_factor(X(idx), V(idx), U(idx), X(idx + 1), V(idx + 1), dt, mass, drag_k, dynamics_noise);
        ...
        gtsam::Vector3 v_dot = - gtsam::Vector3(0, 0, g_) 
                                ...
                                + drag_force / mass
                                + gtsam::Vector3(0, external_force/ mass, 0);
        
        std::cout << "External force is " << external_force << std::endl;
